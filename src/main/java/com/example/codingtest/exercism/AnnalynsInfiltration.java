 ...
 *
 * After some time spent following her best friend's trail, she finds the camp in which her
 * best friend is imprisoned. It turns out there are two kidnappers: a mighty knight and a cunning archer.
 *
 ...
 * Signal prisoner: the prisoner can be signalled using bird sounds if the prisoner is awake and the archer is
 * sleeping, as archers are trained in bird signaling, so they could intercept the message.
 ...
 * If Annalyn does not have her dog then she and the prisoner must be very sneaky! Annalyn can free the
 * prisoner if the prisoner is awake and the knight and archer are both sleeping, but if the prisoner is
 * sleeping they can't be rescued: the prisoner would be startled by Annalyn's sudden appearance and wake
...
public class AnnalynsInfiltration {
     ...
     * method that takes a boolean value that indicates if the knight is awake.
     ...
     * indicating if the knight, archer and the prisoner, respectively, are awake. The method
     ...
     * indicating if the archer and the prisoner, respectively, are awake. The method returns true if the
     * prisoner can be signalled, based on the state of the two characters. Otherwise, returns false
     * @param archerIsAwake
     * @param prisonerIsAwake
     * @return
     */
     ...
     * can be freed based on the state of the three characters and Annalyn's pet dog's presence.
